...
- For competition-oriented end-to-end workflows, default to a custom file-backed `Dataset`, deterministic out-of-fold validation, complete OOF predictions, and fold-model test ensembling as specified in `PRETRAINED_MODEL_POLICY.md`. Use `StratifiedKFold` only when a single stratification label correctly represents independent samples; use group-aware or task-appropriate multilabel/segmentation logic when it does not.
- Treat `train_test_split`, `TensorDataset`, and bare `Subset` as focused teaching tools rather than the default final competition architecture. If a notebook intentionally uses them, state why.
